...
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.CurrentTabHistoryController
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.CurrentTabStrategy
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.FragNavTabHistoryController
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.NavigationStrategy
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.UniqueTabHistoryController
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.UniqueTabHistoryStrategy
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.UnlimitedTabHistoryController
import com.hienthai.vohoctrunghoa.presentation.base.navigator.fragnav.tabhistory.UnlimitedTabHistoryStrategy
 ...
 */
class FragNavController constructor(
    ...
    var navigationStrategy: NavigationStrategy = CurrentTabStrategy
        set(value) {
            ...
            fragNavTabHistoryController = when (value) {
                is UniqueTabHistoryStrategy -> UniqueTabHistoryController(
                    DefaultFragNavPopController(),
                    value.fragNavSwitchController
                )
                is UnlimitedTabHistoryStrategy -> UnlimitedTabHistoryController(
                    DefaultFragNavPopController(),
                    value.fragNavSwitchController
                )
    ...
    var maxLifecycleOnSwitch = false
    ...
    private var fragNavTabHistoryController: FragNavTabHistoryController =
        CurrentTabHistoryController(DefaultFragNavPopController())
    ...
    val currentDialogFrag: DialogFragment?
        ...
        get() {
            if (mCurrentDialogFrag != null) {
                ...
                mCurrentDialogFrag =
                    fragmentManager.fragments.firstOrNull { it is DialogFragment } as DialogFragment?
            }
            return mCurrentDialogFrag
        }
    ...
    private fun switchTabInternal(
        ...
        if (currentStackIndex != index) {
            ...
            if (index == NO_TAB) {
                ...
                if (!shouldAttach && maxLifecycleOnSwitch) {
    ...
    @Throws(UnsupportedOperationException::class)
    private fun tryPopFragmentsFromCurrentStack(
        popDepth: Int,
        transactionOptions: FragNavTransactionOptions?
    ): Int {
    ...
    private fun addPreviousFragment(ft: FragmentTransaction, isAttach: Boolean): Fragment {
        ...
        return if (currentFragment != null) {
            ...
            if (initialSize > 0) {
                val message =
                    "Could not restore any fragment on current stack, adding new root fragment"
                logError(message, IllegalStateException(message))
            }
    ...
    @CheckResult
    private fun createTransactionWithOptions(
        transactionOptions: FragNavTransactionOptions?,
        isPopping: Boolean,
    ...
    inner class DefaultFragNavPopController : FragNavPopController {
        @Throws(UnsupportedOperationException::class)
        override fun tryPopFragments(
            popDepth: Int,
            transactionOptions: FragNavTransactionOptions?
        ): Int {
            return this@FragNavController.tryPopFragmentsFromCurrentStack(
                popDepth,
                transactionOptions
            )
        }
